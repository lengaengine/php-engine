...
final class Vector3 implements ArrayAccess
{
    ...
    public static function lerp(Vector3 $a, Vector3 $b, float $t): self
    {
        ...
        $result = \Lenga\Engine\Core\NativeEngine::call('vector3_lerp', $a->x, $a->y, $a->z, $b->x, $b->y, $b->z, $t);
        if (!\is_array($result)) {
            return new self();
        }
    ...
    {
        /** @var array{x: float, y: float, z: float}|false $result */
        $result = \Lenga\Engine\Core\NativeEngine::call('vector3_slerp', $a->x, $a->y, $a->z, $b->x, $b->y, $b->z, $t);
        if (!\is_array($result)) {
            return new self();
        }
    ...
    public static function smoothDamp(
         ...
         * }|false $result
         */
        $result = \Lenga\Engine\Core\NativeEngine::call('vector3_smooth_damp',
            $current->x,
            $current->y,
            $current->z,
            $target->x,
